...
[[ -n "${season}" ]] && downloadStatus=1 || curl -sf --compressed --connect-timeout 10 -L "https://stats-api.mlssoccer.com/competitions/MLS-COM-000001/seasons" -o "${seasons_file}" && downloadStatus=1 && getSeason

if [[ -n "${downloadStatus}" ]]; then
    # Get season standings
    mkdir -p "${seasonDir}"
    curl -sf --compressed --connect-timeout 10 --parallel \
    ...
    if [[ -f "${seasonDir}/leagueStandings.json" && ! -n ${seasonDir}/icons/*.svg(#qNY1) ]]; then
        ...
        curl -sf --compressed --parallel --output-dir "${seasonDir}/icons" -L "https://images.mlssoccer.com/image/upload/assets/logos/{${teamLogos}}.svg" -o "#1.svg"
    fi
